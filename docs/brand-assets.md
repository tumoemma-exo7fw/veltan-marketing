# Veltan logo assets

The original supplied artwork is archived at
`docs/brand/veltan-logo-source.png`. Production-ready exports are in
`public/brand/`.

## Primary marks

| Asset | Use |
| --- | --- |
| `veltan-mark-light.png` | Full-color transparent mark for white and light surfaces |
| `veltan-mark-dark-color.png` | Lifted teal/sage transparent mark for dark teal/black surfaces |
| `veltan-mark-mono-dark.png` | One-color dark mark for print or limited-color use |
| `veltan-mark-mono-light.png` | One-color light mark for dark backgrounds |

Transparent full-color exports are also available at 64, 128, 256, 512, and
1024 pixels wide for native/mobile applications and other fixed-size uses.

## Application icons

- `veltan-app-icon-{180,192,512}.png` — light app-icon background
- `veltan-app-icon-dark-{180,192,512}.png` — dark app-icon background
- `app/icon.png` — Next.js/browser icon
- `app/apple-icon.png` — iOS home-screen icon

Keep clear space around the mark and do not stretch, recolor, rotate, or add
effects. Use the dark-surface mark on dark backgrounds; do not place the
original dark wing directly on a dark background.
