      ...
      onClick={async () => {
        ...
        try {
          if (!isSupabaseConfigured()) {
            onError(googleAuthError("SUPABASE_NOT_CONFIGURED"));
            setPending(false);
            return;
          }
          ...
          if (error) {
            onError(googleAuthError(error.code, error.message));
            setPending(false);
          }
        } catch (error) {
          const message = error instanceof Error ? error.message : "";
          onError(googleAuthError(undefined, message) || AUTH_NETWORK_ERROR);
          setPending(false);
        }
      }}
